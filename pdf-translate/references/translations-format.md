...
## Context and safe text (version 49)

`segment_targets` maps stable `occurrence_id` values from `to_translate.json`
(for example `p0:s3`) to a target. IDs are zero-based page/local occurrence
numbers and remain stable across page slices of the same source. Numeric legacy
segment IDs remain accepted; conflicting aliases fail. Null occurrence targets
fall back to core translations. Precedence is merge, skip remaining lines,
page override, occurrence target, core target. Do not reuse IDs for a different
source/extraction. `from-cores` scaffolds occurrence slots; `merge-mappings`
combines them and refuses conflicting values by default.

Page text accepts a small HTML subset: b/strong/i/em/u/br/p/div/span/sup/sub,
without attributes, resource URLs, CSS or scripts. The parser rejects unsafe
markup before composition. Escape literal angle brackets as `&lt;`/`&gt;`.
Field strings are literal PDF strings, not HTML; preserve operational values
exactly. The shared effective-text model supplies QA and placement checks.
...
```jsonc
{
  ...
  "translations": {
    ...
    // engine against the four font roles. Safe tags are validated before rendering; escape literal angle brackets.
