...
Version 49: also use [delivery.md](delivery.md) to bind review to final bytes.
A model records `model_bilingual`; it cannot claim human revision. Explicit
`not_performed` entries are required for unavailable review. The checklist
below supplies observations, not proof of a human reviewer or accessibility.
