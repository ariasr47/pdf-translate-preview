...
def _target_of(spec, what, field, allow_empty=False):
    ...
    if isinstance(spec, dict):
        ...
        if not allow_empty and not str(target).strip():
    ...
    if isinstance(spec, list):
        for item in spec:
            ...
            pairs.append((str(item['export']),
                          _target_of(item, f'option {item["export"]}', field, allow_empty=True)))
        return pairs
...
def opt_exports(annot):
    ...
    opt = (annot if isinstance(annot, Field) else Field(annot)).get('/Opt')
    if opt is None:
        return None
...
def choice_exports(path):
    ...
    try:
        for a in iter_fields(pdf):
            ...
            if a.name and exports is not None:
                out[a.name] = exports
    finally:
        pdf.close()
    return out
...
def apply_widget_text(annot, spec, field, report):
    ...
    changed = []
    resolved = Field(annot)
    is_choice = resolved.get('/FT') == Name('/Ch')

    tooltip = _target_of(spec.get('tooltip'), 'tooltip', field)
    if tooltip is not None:
        resolved.owner('/TU')['/TU'] = pikepdf.String(tooltip)
    ...
    if 'options' in spec:
        ...
        if unknown:
            raise WidgetTextError(
                ...
                f'field ({exports})')
        rebuilt = []
        for entry, export in zip(resolved.get('/Opt'), exports):
            display = pairs.get(export)
            if display is None:
                rebuilt.append(entry)
                continue
            source_display = (str(entry[1]) if isinstance(entry, pikepdf.Array)
                              and len(entry) > 1 else str(entry))
            if not display.strip() and source_display.strip():
                raise WidgetTextError(
                    f'{field}: option {export} target is empty for a nonempty source')
            raw_export = entry[0] if isinstance(entry, pikepdf.Array) else entry
            rebuilt.append(pikepdf.Array([raw_export,
                                          pikepdf.String(display)]))
        resolved.owner('/Opt')['/Opt'] = pikepdf.Array(rebuilt)
    ...
    for key, pdfkey in (('value', '/V'), ('default', '/DV')):
        ...
        if target is None:
            continue
        resolved.owner(pdfkey)[Name(pdfkey)] = pikepdf.String(target)
        changed.append(key)
...
def strip_text(src, dst, hide_buttons=None, captions=None, widget_text=None,
    ...
    # Logical fields can exist without a page annotation. Visit the full tree;
    # repeated widget kids also have their stale appearance removed.
    for field in iter_fields(pdf):
        if field.get('/FT') is None:
            continue
        t = field.name
        base = _field_base(t)
        spec_key = t if t in widget_text else (base if base in widget_text else None)
    ...
    for i, page in enumerate(pdf.pages):
        ...
        for a in page.get('/Annots', []):
            ...
            t = resolved.name
            base = _field_base(t)
            if resolved.get('/FT') != Name('/Btn'):
                continue
            ff = int(resolved.get('/Ff', 0) or 0)
