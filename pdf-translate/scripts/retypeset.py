...
def strip_inline_markup(text):
    from text_model import plain_text
    return plain_text(text or '')
...
def apply_document_metadata(doc, document, translations, lang):
    ...
    if lang:
        try:
            # set_language normalizes es-US to es. Preserve the requested
            # BCP-47 tag in the actual catalog, including region/script.
    ...
    if toc:
        ...
        for index, entry in enumerate(toc):
            ...
            if target:
                # Updating only the title preserves destinations, actions,
                # zoom, styling and expansion state of the existing outline.
                doc.set_toc_item(index, title=target)
                changed += 1
        if changed:
...
def canonicalize_text_layer(path, fontfiles, texts):
    ...
    try:
        for obj in pdf.objects:
            ...
            patched += 1
        if patched:
            pdf.save(path, encryption=True if pdf.is_encrypted else False)
    finally:
        pdf.close()
    return patched
...
def retypeset(stripped, segf, trf, out):
    ...
    sidecars = [output_path.parent / name for name in ('scale_report.json', 'build-evidence.json')]
    if not disjoint_files((stripped, segf, trf), [output_path, *sidecars]) or not disjoint_files([output_path], sidecars):
        print('FAIL: output collides with an input or required evidence sidecar')
        return 1
    ...
    if not disjoint_files([p for p in fonts.values() if p], [output_path, *sidecars]):
        print('FAIL: output collides with an input font')
        return 1
    overflow = []
    scaled = []
    ...
    for pno, segs in by_page.items():
        for seg in segs:
            ...
            if text in skip:
                continue
            if segment_target(conf, seg) is None and not seg['passthrough']:
        ...
        for seg in segs:
            ...
            if ov:
                for part in ov['parts']:
                    ...
                    x = part.get('x', ox)
                    t = plain_text(part['text'], strip=False)
            ...
            jp = segment_target(conf, seg)
            if jp is None:
                if not seg['passthrough']:
                    missing.append((pno, core))
                f = helv_role(seg['bold'], seg.get('italic'))
                raw = seg['text'].rstrip()
                face_label = 'Helvetica (pass-through)'
                if missing_glyphs(f, raw):
                    f = role(seg['bold'], seg.get('italic'), core)
                    face_label = 'configured role (pass-through)'
                check_glyphs(pno, f, raw, raw, face_label)
                if rot:
    ...
    doc.ez_save(out, encryption=pymupdf.PDF_ENCRYPT_KEEP)
    doc.close()

    # The glyphs are right either way; this is the text layer.
    placed = [r['target'] for r in effective_texts(conf, segments)]
    ...
    write_build(out, stripped, segf, trf, {
        ...
        'italic': f_italic, 'bold_italic': f_bold_italic}, report)
