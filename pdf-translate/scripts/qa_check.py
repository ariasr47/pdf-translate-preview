...
def qa_check(translations_path, segments_path=None, glossary_path=None, widget_text_path=None):
    ...
    try:
        widget_text = None
        if widget_text_path:
            with open(widget_text_path, encoding='utf-8') as f:
                widget_text = json.load(f)
        rows = effective_texts(conf, segments, widget_text)
    ...
    for row in rows:
        if (row['channel'] == 'notice' and not row.get('source_supplied')) or row.get('source_supplied') is False:
            findings.append(_finding('source-unavailable', 'warn', '',
                f"{row['channel']} has no source text; protected-data comparison requires source"))
            continue
        pair = check_pair(row['source'], row['target'], identifiers, target_script, normalized=True)
        if row.get('protected') and row['source'] != row['target']:
            pair.append(_finding('protected-data','error',row['source'],
                'widget operational value/default must remain identical'))
        pair.extend(glossary_findings({row['source']: row['target']}, terms))
        for finding in pair:
            finding['channel'] = row['channel']
            if 'segment_id' in row: finding['segment_id'] = row['segment_id']
            if 'page' in row: finding['page'] = row['page']
