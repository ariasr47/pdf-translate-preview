...
def job_charset(conf):
    ...
    for v in (conf.get('segment_targets') or {}).values():
        if v is not None:
            chars.update(plain_text(v))
    ...
    for m in conf.get('merges') or []:
        chars.update(plain_text(m.get('html') or ''))
    for o in conf.get('overrides') or []:
        for part in o.get('parts') or []:
            chars.update(plain_text(part.get('text') or ''))
    for n in conf.get('notices') or []:
        chars.update(plain_text(n.get('text') or ''))
