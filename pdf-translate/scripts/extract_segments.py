...
# (benchmarks/wild/ANALYSIS.md): within one em the proposals fall from 1,485 groups
...
def extract_segments(src, outdir='.', gap=12.0, pages=None):
    ...
    page_occurrences = {}
    for seg in segments:
        local = page_occurrences.get(seg['page'], 0)
        seg['occurrence_id'] = f"p{seg['page']}:s{local}"
        page_occurrences[seg['page']] = local + 1
    uniq = {}
    for s in segments:
    ...
    with open(seg_path, 'w', encoding='utf-8') as f:
        ...
        json.dump({'source': src, 'source_sha256': sha256(src), 'segments': segments,
                   'warnings': warnings, 'document': document,
                   'pages': sorted(wanted)},
                  f, ensure_ascii=False, indent=1)
    contexts = []
    for index, seg in enumerate(segments):
        same_page = lambda other: other['page'] == seg['page']
        contexts.append({'id': seg['id'], 'occurrence_id': seg['occurrence_id'], 'source': seg['core'],
                         'page': seg['page'],
                         'role': 'bold' if seg.get('bold') else 'regular',
                         'previous': segments[index-1]['text'] if index and same_page(segments[index-1]) else None,
                         'next': segments[index+1]['text'] if index+1 < len(segments) and same_page(segments[index+1]) else None,
                         'target': None})
    cores = [{'text': k, 'count': v} for k, v in uniq.items()]
    with open(to_path, 'w', encoding='utf-8') as f:
        json.dump({'cores': cores, 'occurrences': contexts}, f, ensure_ascii=False, indent=1)
    return {
        'segments': segments,
        'warnings': warnings,
        'cores': cores,
...
# at a time, and the one that mattered scrolled past (benchmarks/wild/ANALYSIS.md
