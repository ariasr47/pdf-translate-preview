...
def collect_translation_targets(conf, exclude_cores=None, segments=None):
    ...
    return [target for row in effective_texts(conf, segments)
            if row['source'] not in exclude and row['channel'] != 'widget'
            for target in row['placement_targets']]
...
def document_metadata_misses(odoc, jdoc, conf):
    ...
    try:
        kind, raw_lang = jdoc.xref_get_key(jdoc.pdf_catalog(), 'Lang')
        got_lang = raw_lang.strip() if kind == 'string' else ''
    except Exception:
        got_lang = ''
    if want_lang:
        if not got_lang or want_lang.lower().replace('_', '-') != got_lang.lower().replace('_', '-'):
...
def empty_translation_targets(conf):
    ...
    for core, tgt in (conf.get('translations') or {}).items():
        ...
        if _authored_text_is_empty(tgt):
            add(core)
    for sid, tgt in (conf.get('segment_targets') or {}).items():
        if tgt is not None and _authored_text_is_empty(tgt):
            add(f'segment {sid}')
...
def missing_identifier_spans(page_text, spans, allow_translate=None):
    ...
    """
    allow = {normalize_ws(normalize_ws_nbsp(s)) for s in (allow_translate or []) if s}
    hay = normalize_ws(normalize_ws_nbsp(page_text))
    missing = []
    for raw in spans:
        nt = normalize_ws(normalize_ws_nbsp(raw))
        if nt in allow:
            continue
        if nt not in hay:
            missing.append(raw)
...
def unicode_words(text):
    """Keep translated letter/mark words as barriers between source runs."""
    word = []
    for ch in unicodedata.normalize('NFC', text):
        category = unicodedata.category(ch)
        if category.startswith('L') or (word and (category.startswith('M') or ch in "'-")):
            word.append(ch)
        elif word:
            yield ''.join(word)
            word = []
...
def verify(orig, trans, fill_text='Test value 123', allow=None, min_ink=0.4,
    ...
    if translations:
        with open(translations, encoding='utf-8') as f:
            mirror = bool(json.load(f).get('mirror'))
    structural = structural_misses(orig, trans, mirror=mirror,
                                   allow_extra_prefix=allow_extra_prefix)
    for detail in structural:
        print('FAIL structural parity: ' + detail)
    if structural:
        fail = 1
    else:
        print('PASS structural parity (pages, geometry, field occurrences)')
    ...
    if translations:
        with open(translations, encoding='utf-8') as f:
            _conf = json.load(f)
        authored_segments = load_segments_file(translations, segments)
        try:
            authored += '\n' + '\n'.join(collect_translation_targets(
                _conf, segments=(authored_segments or {}).get('segments')))
        except ValueError as exc:
            print(f'FAIL authored mapping: {exc}')
            fail = 1
    ...
    if translations:
        ...
        try:
            targets = collect_translation_targets(conf, exclude_cores=meta_cores,
                segments=(segfile or {}).get('segments'))
        except ValueError as exc:
            print(f'FAIL authored mapping: {exc}')
            fail = 1
            targets = []
